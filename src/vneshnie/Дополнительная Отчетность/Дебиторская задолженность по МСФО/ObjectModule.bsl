Функция СформироватьЗапрос() Экспорт
	...
	Если БезДоговоров Тогда
		...
		|	ВЫБОР
		|		КОГДА &Дебиторы
		|			ТОГДА ЕСТЬNULL(МСФООстатки.СуммаОстатокДт, 0) - ЕСТЬNULL(МСФООстатки.СуммаОстатокКт, 0)
		|		ИНАЧЕ ЕСТЬNULL(МСФООстатки.СуммаОстатокКт, 0) - ЕСТЬNULL(МСФООстатки.СуммаОстатокДт, 0)
		|	КОНЕЦ КАК СуммаЗадолженности,
		|	ВЫБОР
		|		КОГДА &Дебиторы
		|			ТОГДА ЕСТЬNULL(МСФООбороты.СуммаОборотДт, 0) - ЕСТЬNULL(МСФООбороты.СуммаОборотКт, 0)
		|		ИНАЧЕ ЕСТЬNULL(МСФООбороты.СуммаОборотКт, 0) - ЕСТЬNULL(МСФООбороты.СуммаОборотДт, 0)
		|	КОНЕЦ КАК СуммаОборот,
		|	ВЫБОР
		|		КОГДА &Дебиторы
		|			ТОГДА ЕСТЬNULL(МСФООстаткиНачало.СуммаОстатокДт, 0) - ЕСТЬNULL(МСФООстаткиНачало.СуммаОстатокКт, 0)
		|		ИНАЧЕ ЕСТЬNULL(МСФООстаткиНачало.СуммаОстатокКт, 0) - ЕСТЬNULL(МСФООстаткиНачало.СуммаОстатокДт, 0)
		...
		|					ИЛИ Субконто1 В ИЕРАРХИИ (&Контрагенты))) КАК МСФООстатки
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.МСФО.Обороты(
		|				&ДатаНачала,
		|				&Дата,
		|				Месяц,
		|				Счет В ИЕРАРХИИ (&Счета),
		...
		|				,
		|				) КАК МСФООбороты
		|		ПО МСФООстатки.Субконто1 = МСФООбороты.Субконто1
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.МСФО.Остатки(
		|				&ДатаНачала,
		|				Счет В ИЕРАРХИИ (&Счета),
		|				,
		|				Организация = &Организация
		|					И (&ПоВсемКонтрагентам
		|						ИЛИ Субконто1 В ИЕРАРХИИ (&Контрагенты))) КАК МСФООстаткиНачало
		|		ПО МСФООстатки.Субконто1 = МСФООстаткиНачало.Субконто1
		|ГДЕ
		|	ВЫБОР
		|			КОГДА &Дебиторы
		|				ТОГДА ЕСТЬNULL(МСФООстатки.СуммаОстатокДт, 0) - ЕСТЬNULL(МСФООстатки.СуммаОстатокКт, 0)
	...
	Иначе
		...
		|				) КАК МСФООбороты
		|		ПО МСФООстатки.Субконто1 = МСФООбороты.Субконто1
		|			И МСФООстатки.Субконто2 = МСФООбороты.Субконто2
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.МСФО.Остатки(
		|				&ДатаНачала,
		|				Счет В ИЕРАРХИИ (&Счета),
		|				,
		...
		|						ИЛИ Субконто1 В ИЕРАРХИИ (&Контрагенты))) КАК МСФООстаткиНачало
		|		ПО МСФООстатки.Субконто1 = МСФООстаткиНачало.Субконто1
		|			И МСФООстатки.Субконто2 = МСФООстаткиНачало.Субконто2
		|ГДЕ
		|	ВЫБОР
		|			КОГДА &Дебиторы
		|				ТОГДА ЕСТЬNULL(МСФООстатки.СуммаОстатокДт, 0) - ЕСТЬNULL(МСФООстатки.СуммаОстатокКт, 0)
...
Процедура СформироватьОтчет() Экспорт 
	...
	УниверсальныеМеханизмы.НапечататьДокумент(Результат,,,"Задолженность контрагентов по МСФО");
