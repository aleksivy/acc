...
//	|	Ответственный.ФизЛицо.Наименование КАК Отпустил,
	...
	Макет = ПолучитьМакет("КорректировкаРеализации");
	...
	ОбластьМакета.Параметры.ТекстЗаголовка = НСтр("ru='Расчет изменения суммы компенсации за ранее отгруженный товар';uk='Розрахунок зміни суми компенсації за раніше поставлений товар'",КодЯзыкаПечать) + Символы.ПС
	+ "№ " + ОбщегоНазначения.ПолучитьНомерНаПечать(СсылкаНаОбъект) + НСтр("ru=' от ';uk=' від '",КодЯзыкаПечать) + Формат(СсылкаНаОбъект.Дата, "ДФ='дд ММММ гггг';Л="+ Локализация.ОпределитьКодЯзыкаДляФормат(КодЯзыкаПечать)) + НСтр("ru=' г.';uk=' р.'",КодЯзыкаПечать);
						
	ОбластьМакета.Параметры.ТекстЗаголовка2 = НСтр("ru='к расходной накладной';uk='до видаткової накладної'",КодЯзыкаПечать) + " № " + ОбщегоНазначения.ПолучитьНомерНаПечать(СсылкаНаОбъект.ДокументОснование)
	                          + НСтр("ru=' от ';uk=' від '",КодЯзыкаПечать) + Формат(СсылкаНаОбъект.ДокументОснование.Дата, "ДФ='дд ММММ гггг';Л="+ Локализация.ОпределитьКодЯзыкаДляФормат(КодЯзыкаПечать)) + НСтр("ru=' г.';uk=' р.'",КодЯзыкаПечать);
	...
	ОбластьМакета.Параметры.РеквизитыПокупателя		= ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПокупателе,"КодПоЕДРПОУ,ФактическийАдрес,Телефоны,",,КодЯзыкаПечать);
	ТабДокумент.Вывести(ОбластьМакета);
	...
	ОбластьМакета = Макет.ПолучитьОбласть("ТекстДоШапки");
    ТабДокумент.Вывести(ОбластьМакета);
	...
	КонецЕсли;
    ОбластьДанных.Параметры.Цена  = НСтр("ru='(+,-) к цене поставки';uk='(+,-) до ціни постачання'",КодЯзыкаПечать); //  + Суффикс;
...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Наименование", "Печатная форма Корректировка реализации"); //имя под kt обработка зарегистрирована будет в справочнике внешних обработок
	ПараметрыРегистрации.Вставить("Версия", "1.0");
	ПараметрыРегистрации.Вставить("БезопасныйРежим", ЛОЖЬ);
	ПараметрыРегистрации.Вставить("Информация", "Печатная форма Корректировка реализации");
	//команды
	...
	НоваяКоманда.Представление = "Печатная форма Корректировка реализации";
	НоваяКоманда.Идентификатор = "ПечатьКорректировкаРеализации"; //Внешняя печатная форма
