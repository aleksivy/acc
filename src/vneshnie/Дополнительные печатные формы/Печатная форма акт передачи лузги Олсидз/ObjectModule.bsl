Функция Печать() Экспорт
	...
	ОбластьМакета.Параметры.НазваниеОрганизацииПолноеВРег = ВРег( ггуОбщегоНазначения.НазваниеОрганизации(лЭтотОбъект.Дата, Истина, Ложь, Ложь) );
	ОбластьМакета.Параметры.НазваниеОрганизации			  = ВРег( ггуОбщегоНазначения.НазваниеОрганизации(лЭтотОбъект.Дата, Ложь, Ложь, Ложь) );
	лНомерДоговора = СокрЛП(лЭтотОбъект.ДоговорКонтрагента.Номер);
	лПоз = Найти(лНомерДоговора, "/");
	Если лПоз > 0 Тогда
		лНомерДоговора = Прав(лНомерДоговора, СтрДлина(лНомерДоговора) - лПоз);
	КонецЕсли;
	ОбластьМакета.Параметры.НомерДоговора				  = лНомерДоговора;
	ОбластьМакета.Параметры.ДатаДоговора				  = Формат(лЭтотОбъект.ДоговорКонтрагента.Дата, "ДФ=dd.MM.yyyy");
