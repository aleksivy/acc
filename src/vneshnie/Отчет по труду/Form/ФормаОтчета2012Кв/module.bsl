...
Процедура ЗаполнитьНачальныеНастройки(Макет = "СоставПоказателей2012Кв") Экспорт
...
Функция ОбновитьВариантыЗаполнения()
	...
	МакетСоставаПоказателей = ЭтотОбъект.ПолучитьМакет("СоставПоказателей2012Кв");
...
Процедура РасчетПоказателейКоличестваРаботников()
	...
	|	Количество(ВЫБОР КОГДА Сотрудник.ФизЛицо.Пол = ЗНАЧЕНИЕ(Перечисление.ПолФизическихЛиц.Женский) ТОГДА 1 ИНАЧЕ 0 КОНЕЦ) КАК КоличествоЖенщин
	|
	|ИЗ
...
Процедура ВосстановитьГраницыРаспределенияРаздел3()
	...
	Если мДатаКонцаПериодаОтчета >= '2012-06-30' Тогда
...
мСоставПоказателей = "СоставПоказателей2012Кв";

// ОПИСАНИЕ ПАРМЕТРОВ ФОРМЫ
//
// Версия формы
мВерсияФормы = "01/01/2012";
