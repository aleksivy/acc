...
Процедура ПоказатьПериод()
	...
	Если МесяцРасчета = 12 ИЛИ МесяцРасчета = 3 ИЛИ МесяцРасчета = 6 ИЛИ МесяцРасчета = 9 Тогда
		МесяцЗадол = ДобавитьМесяц(мДатаКонцаПериодаОтчета,1);
	Иначе
		МесяцЗадол = НачалоКвартала(мДатаКонцаПериодаОтчета);
	КонецЕсли;	
...
Процедура ЗаполнитьАвто()   Экспорт
	...
	|ИЗ                                                                                                  
	|   РегистрНакопления.ВзаиморасчетыПоНДФЛ.Обороты(&ДатаНачалаНачислений, &КонДата,МЕСЯЦ , Организация = &Организация И ДоходНДФЛ = &ДоходФОТ
	|																				    И ПериодВзаиморасчетов >= &НачДата
	...
	НачДата = НачалоКвартала(мДатаКонцаПериодаОтчета + 1);
