...
//
Процедура ЗаполнитьНачальныеНастройки(Макет = "СоставПоказателей2008Мес") Экспорт
...
Функция ПолучитьТаблицуВариантовЗаполнения(ИмяСтраницыПанели, ПолучитьЭталонную = Ложь)
	...
	Если мСтруктураВариантыЗаполнения.Свойство(ИмяСтраницыПанели, МассивВариантовЗаполненияСтраниц) Тогда
		...
		Иначе
			...
			МакетСоставаПоказателей = ЭтотОбъект.ПолучитьМакет("СоставПоказателей2008Мес");
...
Процедура ПоказатьПериод()
	...
	мОтчетПоТруду.Области.ГодОтчета.Значение = Прав (ГодОтчета,1);
...
Процедура ЗаполнитьАвто()   Экспорт
	...
	|ВЫБРАТЬ
	|  СУММА (СуммаВзаиморасчетовПриход) КАК РезультатВсе,
	...
	|";
	ЗапросОбшийФОТ.УстановитьПараметр("НачДата", мДатаНачалаПериодаОтчета);
	ЗапросОбшийФОТ.УстановитьПараметр("КонДата", мДатаКонцаПериодаОтчета);
	ЗапросОбшийФОТ.УстановитьПараметр("ДатаНачалаНачислений", ДатаНачалаНачисленийТекПериода(мДатаНачалаПериодаОтчета, мДатаКонцаПериодаОтчета, Организация, "ВзаиморасчетыСРаботникамиОрганизаций"));
	ЗапросОбшийФОТ.УстановитьПараметр("Организация",   Организация);
	ТаблицаМес = ЗапросОбшийФОТ.Выполнить().Выгрузить();
	УстановитьЗначениеПоказателя("П1020_1",ТаблицаМес[0].РезультатВсе);
	УстановитьЗначениеПоказателя("П1070_1",ТаблицаМес[0].РезультатШтат);
	...
	УстановитьЗначениеПоказателя("П1020_2",ТаблицаНачГод[0].РезультатВсе);
	УстановитьЗначениеПоказателя("П1070_2",ТаблицаНачГод[0].РезультатШтат);
	...
	|ИЗ                                                                                                  
	|   РегистрНакопления.ВзаиморасчетыПоНДФЛ.Обороты(&ДатаНачалаНачислений, &КонДата,МЕСЯЦ , Организация = &Организация И ДоходНДФЛ В (&ДоходФОТ)
	|																				    И ПериодВзаиморасчетов >= &НачДата
	...
	ДоходФОТ = Новый Массив();
	ДоходФОТ.Добавить(Справочники.ВидыДоходовНДФЛ.Код01);
	ДоходФОТ.Добавить(Справочники.ВидыДоходовНДФЛ.Код42);
	Попытка
		ДоходФОТ.Добавить(Справочники.ВидыДоходовНДФЛ.Код01ГПХ);
	Исключение	
	КонецПопытки;
	ЗапросНДФЛ.УстановитьПараметр("ДоходФОТ",ДоходФОТ);
	...
	Сообщить(НСтр("ru='Невозможно автоматически исключать из состава строк 2010 и 2030 сумму задолженности по больничному!';uk='Неможливо автоматично виключати зі складу рядків 2010 та 2030 суму заборгованості по лікарняним!'"));
	...
	|	Сумма(ВЫБОР КОГДА Сотрудник.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятостиВОрганизации.ОсновноеМестоРаботы) Тогда СуммаВзаиморасчетовОстаток ИНАЧЕ 0 КОНЕЦ) КАК РезультатШтат,
	|	Количество(Различные ВЫБОР КОГДА Сотрудник.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятостиВОрганизации.ОсновноеМестоРаботы) Тогда Сотрудник ИНАЧЕ NULL КОНЕЦ) КАК РезультатШтатКоличество
	|ИЗ
	...
	Если РезультатЗапроса.Следующий() Тогда
		...
		УстановитьЗначениеПоказателя("П2030_1",РезультатЗапроса.РезультатШтат);
		УстановитьЗначениеПоказателя("П2040_1",РезультатЗапроса.РезультатШтатКоличество);
	КонецЕсли;
	
	//////////////////////////////////////////////////////
	ЗапросКолМесЗадолженности = Новый Запрос;
	ЗапросКолМесЗадолженности.Текст = 
	"ВЫБРАТЬ 
	|	ВзаиморасчетыСРаботниками.Сотрудник КАК Сотрудник
	|ИЗ
	|	РегистрНакопления.ВзаиморасчетыСРаботникамиОрганизаций.Остатки(&НачДата,  Организация = &Организация
	|																			И СчетУчета = &парамСчет
	|																			И ПериодВзаиморасчетов < &ПериодЗадолженности
	|																			И Сотрудник.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятостиВОрганизации.ОсновноеМестоРаботы)) КАК ВзаиморасчетыСРаботниками
	|ГДЕ
	|	ВзаиморасчетыСРаботниками.СуммаВзаиморасчетовОстаток > 0
	|
	|СГРУППИРОВАТЬ ПО
	...
	|
	|ИМЕЮЩИЕ	
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ НАЧАЛОПЕРИОДА(ВзаиморасчетыСРаботниками.ПериодВзаиморасчетов, МЕСЯЦ)) < 3
	|
	|ИТОГИ 
	...
	|";
	НачДата = мДатаКонцаПериодаОтчета + 1;
	ЗапросКолМесЗадолженности.УстановитьПараметр("НачДата", НачДата);
	ЗапросКолМесЗадолженности.УстановитьПараметр("ПериодЗадолженности", ДобавитьМесяц(НачДата,-1));
	ЗапросКолМесЗадолженности.УстановитьПараметр("Организация",   Организация);
	ЗапросКолМесЗадолженности.УстановитьПараметр("парамСчет", Счет661);
	РезультатЗапроса = ЗапросКолМесЗадолженности.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	Если РезультатЗапроса.Следующий() Тогда
		УстановитьЗначениеПоказателя("П2050_1",РезультатЗапроса.Сотрудник);
	КонецЕсли;
...
ЭлементыФормы.ПолеВыбораВариантЗаполненияОтчетПоТрудуМес.СписокВыбора.Добавить(2, НСтр("ru='заполнять автоматически с корректировкой';uk='заповнювати автоматично з коректуванням'"));
ЭлементыФормы.ПолеВыбораВариантЗаполненияОтчетПоТрудуМЕс.СписокВыбора.Добавить(3, НСтр("ru='заполнять автоматически без корректировки';uk='заповнювати автоматично без коректування'"));
...
мСоставПоказателей = "СоставПоказателей2008Мес";
