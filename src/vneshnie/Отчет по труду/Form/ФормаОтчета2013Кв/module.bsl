...
Процедура ЗаполнитьНачальныеНастройки(Макет = "СоставПоказателей2013Кв") Экспорт
...
Функция ПолучитьТаблицуВариантовЗаполнения(ИмяСтраницыПанели, ПолучитьЭталонную = Ложь)
	...
	Если мСтруктураВариантыЗаполнения.Свойство(ИмяСтраницыПанели, МассивВариантовЗаполненияСтраниц) Тогда
		...
		Иначе
			...
			МакетСоставаПоказателей = ЭтотОбъект.ПолучитьМакет("СоставПоказателей2013Кв");
...
Процедура ЗаполнитьСведенияОбОрганизации()
	...
	мОтчетПоТруду.Области.ДатаРаздел7.Значение   =  "Усього по підприємству
													 |за січень - грудень
													 |" + Формат(Год(мДатаКонцаПериодаОтчета)-1, "ЧДЦ=0; ЧГ=0")+ " р.";
...
Процедура ОбновитьПользовательскиеНастройки()
	...
	"Виконується відновлення користувальницьких настроювань.'"), СтатусСообщения.Информация);
	...
	Если ЕстьИзмененияВариантыЗаполнения Тогда
		...
		Сообщить(НСтр("ru='Обновление пользовательских настроек успешно завершено!';uk='Оновлення користувальницьких настроювань успішно завершене!'"));
		
	Иначе
		
		Сообщить(НСтр("ru='Изменения в структуре пользовательских настроек не обнаружены!';uk='Зміни в структурі користувальницьких настроювань не виявлені!'"));
...
Процедура Очистить()
	...
	// сохраним настройку
	Пр100 = мОтчетПоТруду.Области.Пр100.Значение; 
	Пр2   = мОтчетПоТруду.Области.Пр2.Значение; 
	
	СохранитьГраницыРаспределенияРаздел3();
	ОчиститьТабличноеПоле(ТекТабличноеПоле);
	
	ВосстановитьГраницыРаспределенияРаздел3();
	// восстановим настройку
	мОтчетПоТруду.Области.Пр100.Значение = Пр100; 
	мОтчетПоТруду.Области.Пр2.Значение = Пр2; 
...
Процедура Печать(ВидПечати)
	...
	ОбластьПечати = мОтчетПоТруду.ПолучитьОбласть("Отчет3");
	ПечатнаяФорма.Вывести(ОбластьПечати);
...
Процедура Расчет()
   ...
   мОтчетПоТруду.Области.П9010.Значение   =   ОкруглитьЧислоПоФормату(мОтчетПоТруду.Области.П9020.Значение)
											+ ОкруглитьЧислоПоФормату(мОтчетПоТруду.Области.П9030.Значение)
											+ ОкруглитьЧислоПоФормату(мОтчетПоТруду.Области.П9040.Значение)
											+ ОкруглитьЧислоПоФормату(мОтчетПоТруду.Области.П9050.Значение)
											+ ОкруглитьЧислоПоФормату(мОтчетПоТруду.Области.П9060.Значение);
...
Процедура РасчетНачисленийШтатныхРаботников()
	...
	|  ПО	Отработка.Сотрудник = ВзаиморасчетыСРаботниками.Сотрудник
	|
	|ГДЕ 
...
Процедура РасчетПоказателейКоличестваРаботников()
	...
	|	СУММА(ВЫБОР КОГДА Сотрудник.ФизЛицо.Пол = ЗНАЧЕНИЕ(Перечисление.ПолФизическихЛиц.Женский) ТОГДА 1 ИНАЧЕ 0 КОНЕЦ) КАК КоличествоЖенщин
	|
	|ИЗ
...
Процедура РасчетИспользованиеРабочегоВремениШтатныхРаботников()
	...
	Если    РегламентированнаяОтчетность.ИДКонфигурации() = "УПП" 
	    ИЛИ РегламентированнаяОтчетность.ИДКонфигурации() = "УТП"
		ИЛИ РегламентированнаяОтчетность.ИДКонфигурации() = "ЗУП" Тогда 
	...
	|	И (    ОсновныеНачисленияРаботниковОрганизации.Сотрудник.ВидЗанятости = &ОсновноеМестоРаботы
	|      ИЛИ ОсновныеНачисленияРаботниковОрганизации.Сотрудник.ВидЗанятости = &ВнутреннееСовместительство)
...
Процедура  РасчетФОТиПрочихВыплат()
	...
	ТекстЗапроса =  "ВЫБРАТЬ
					...
					|	И (    Начисления.Сотрудник.ВидЗанятости = &ОсновноеМестоРаботы
					|      ИЛИ Начисления.Сотрудник.ВидЗанятости = &ВнутреннееСовместительство)
...
Процедура РаспределениеРаботниковПоРазмеруЗП()
	...
	Если РегламентированнаяОтчетность.ИДКонфигурации() = "БП" Тогда
		...
		УсловиеЭтоОклад = "И ВидРасчетаИзмерение.ЭтоОклад";
		Размер = "Размер";
		...
		|	СУММА(ВЫБОР
		|			КОГДА Календарь.Пятидневка <> 0
		|				ТОГДА 1
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК НормаДнейЗаМесяц,
		|	СУММА(Календарь.Пятидневка) КАК НормаЧасовЗаМесяц
		|ПОМЕСТИТЬ ВТГрафикиРаботы
		|ИЗ
		|	РегистрСведений.РегламентированныйПроизводственныйКалендарь КАК Календарь
		|ГДЕ
		|	КОНЕЦПЕРИОДА(Календарь.ДатаКалендаря, МЕСЯЦ) = &ДатаНач;
		|";
		
	Иначе	
		
		УсловиеЭтоОклад = "(ВидРасчетаИзмерение = &ПустоеЗначениеВР ИЛИ ВидРасчетаИзмерение = НЕОПРЕДЕЛЕНО)";
		Размер  = "Показатель1";
		ВидРасчета  = "ВидРасчета";
		УсловиеАктуальность  = "Актуальность И ";
	...
	|	ВЫБОР
	|		КОГДА Начисления." + ВидРасчета + ".СпособРасчета = ЗНАЧЕНИЕ(Перечисление.СпособыРасчетаОплатыТруда.ПоМесячнойТарифнойСтавкеПоДням)
	...
	|				"+ УсловиеЭтоОклад+ "
	|				И Сотрудник.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятостиВОрганизации.ОсновноеМестоРаботы)) КАК Начисления
	|               " + РазмерыТарифныхСтавок + "
	|ИНДЕКСИРОВАТЬ ПО
	|	Сотрудник
	|;
...
Процедура ЗаполнитьАвто()  Экспорт
	...
	Если Месяц(КонецКвартала(мДатаКонцаПериодаОтчета)) = 3 Тогда 
		ДатаПрошлогоГода = ДобавитьМесяц(мДатаКонцаПериодаОтчета, -12);
		ТаблСреднеспЧисленности = ПроведениеРасчетов.ПолучитьСреднююЧисленность(НачалоГода(ДатаПрошлогоГода),КонецГода(ДатаПрошлогоГода),"ССЧ","КВАРТАЛСНАЧАЛАГОДА",Организация, "");
		мОтчетПоТруду.Области.П9070.Значение =  ТаблСреднеспЧисленности[0].Средняя;
	КонецЕсли;  
...
Функция ПроверитьГраницыРаспределенияРаздел4()
	...
	Если мОтчетПоТруду.Области.Пр10.Значение <> мОтчетПоТруду.Области.Пр900.Значение Тогда
	    Результат = Ложь;
	КонецЕсли;
	...
	Если Результат = Ложь И (мМенеджерЗвит1С = Неопределено ИЛИ Не мМенеджерЗвит1С.ФлагОтладкиПакетноеТестирование) Тогда
		Сообщить(НСтр("ru = 'Не заполнены (или не правильно заполнены) параметры распределения размера заработной платы в разделе IV.
                       |Строки 6020...6110 заполнены не будут!'; uk = 'Не заповнені (або заповнені некоректно) параметри розподілу розміру зарабітної плати в разділі IV.
                       |Рядки 6020...6100 заповнені не будуть!'"));	
	КонецЕсли;
...
Процедура ВосстановитьГраницыРаспределенияРаздел3()
	...
	мОтчетПоТруду.Области.Пр8.Значение    = 4000.01;
	мОтчетПоТруду.Области.Пр9.Значение    = 5000.01;
	мОтчетПоТруду.Области.Пр10.Значение   = 10000.00;
	...
	мОтчетПоТруду.Области.Пр700.Значение  = 4000.00;
	мОтчетПоТруду.Области.Пр800.Значение  = 5000.00;
	мОтчетПоТруду.Области.Пр900.Значение  = 10000.00;
	
	// не заданы в эталонной форме форме
	Если НЕ ЗначениеЗаполнено(мОтчетПоТруду.Области.Пр100.Значение) Тогда
		
		МинЗП = 0;
		Попытка
			РС = РегистрыСведений.МинимальнаяОплатаТруда.СрезПоследних(НачалоМесяца(мДатаКонцаПериодаОтчета));
			Если РС.Количество() > 0 Тогда
				МинЗП = РС[0].Размер;
			КонецЕсли;
		Исключение
			МинЗП = ВосстановитьЗначение("РегламентированнаяОтчетность_ОтчетПоТруду_Пр100");
		КонецПопытки;	
		
		Если ЗначениеЗаполнено(МинЗП) Тогда
			мОтчетПоТруду.Области.Пр2.Значение    = МинЗП + 0.01;
			мОтчетПоТруду.Области.Пр100.Значение  = МинЗП;
		КонецЕсли;
...
Процедура РаспределениеДоПриИзменении(Элемент)
	...
	Если НЕ ИмяПоказателя = "Пр900" Тогда
...
ЭлементыФормы.ПолеВыбораВариантЗаполненияОтчетПоТруду.СписокВыбора.Добавить(2, НСтр("ru='заполнять автоматически с корректировкой';uk='заповнювати автоматично з коректуванням'"));
ЭлементыФормы.ПолеВыбораВариантЗаполненияОтчетПоТруду.СписокВыбора.Добавить(3, НСтр("ru='заполнять автоматически без корректировки';uk='заповнювати автоматично без коректування'"));
...
мСоставПоказателей = "СоставПоказателей2013Кв";

// ОПИСАНИЕ ПАРМЕТРОВ ФОРМЫ
//
// Версия формы
мВерсияФормы = "01/01/2013";
