...
//
Процедура ЗаполнитьНачальныеНастройки(Макет = "СоставПоказателей2012Мес") Экспорт
...
Функция ПолучитьТаблицуВариантовЗаполнения(ИмяСтраницыПанели, ПолучитьЭталонную = Ложь)
	...
	Если мСтруктураВариантыЗаполнения.Свойство(ИмяСтраницыПанели, МассивВариантовЗаполненияСтраниц) Тогда
		...
		Иначе
			...
			МакетСоставаПоказателей = ЭтотОбъект.ПолучитьМакет("СоставПоказателей2012Мес");
...
Процедура ПоказатьПериод()
	...
	мОтчетПоТруду.Области.ПериодОтчета.Значение = "За січень - " + ПериодОтчета;
...
Процедура ЗаполнитьАвто()   Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
					...
					|		КОГДА Сотрудник.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятостиВОрганизации.ВнутреннееСовместительство) 
					|         ТОГДА Начисления.Результат
					|		  ИНАЧЕ 0
					|   КОНЕЦ КАК СуммаШтат,
					|	ВЫБОР
	...
	|	Начисления."+УсловиеПериод+" МЕЖДУ &НачалоПериода И &КонецПериода
	|	И Начисления.Организация = &парамОрганизация
	|	И Начисления.Сотрудник.ВидЗанятости В(&ВидыЗанятостиСотрудниковШтат)
	...
	ЗапросРабочееВремя.УстановитьПараметр("НачалоПериода", мДатаНачалаПериодаОтчета);
	ЗапросРабочееВремя.УстановитьПараметр("КонецПериода" , мДатаКонцаПериодаОтчета);
	ЗапросРабочееВремя.УстановитьПараметр("парамОрганизация",  Организация);
	
	ВидыЗанятостиСотрудниковШтат = Новый Массив;
	ВидыЗанятостиСотрудниковШтат.Добавить(Перечисления.ВидыЗанятостиВОрганизации.ОсновноеМестоРаботы);
	ВидыЗанятостиСотрудниковШтат.Добавить(Перечисления.ВидыЗанятостиВОрганизации.ВнутреннееСовместительство);
	...
	Если ТаблицаРабочееВремя.Количество() > 0 Тогда
		УстановитьЗначениеПоказателя("П1050_1", ТаблицаРабочееВремя[0].КвоЧасов);		
		УстановитьЗначениеПоказателя("П1060_1", ТаблицаРабочееВремя[0].КвоЧасовОтработано);	
	КонецЕсли;
	
	ЗапросРабочееВремя.УстановитьПараметр("НачалоПериода", НачалоГода(мДатаНачалаПериодаОтчета));
	
 	ТаблицаРабочееВремя = ЗапросРабочееВремя.Выполнить().Выгрузить();
	Если ТаблицаРабочееВремя.Количество() > 0 Тогда
		УстановитьЗначениеПоказателя("П1050_2", ТаблицаРабочееВремя[0].КвоЧасов);		
		УстановитьЗначениеПоказателя("П1060_2", ТаблицаРабочееВремя[0].КвоЧасовОтработано);	
	КонецЕсли;
	...
	ТекстСообщения = НСтр("ru='Невозможно автоматически исключать из состава строк 2010 и 2020 сумму задолженности по больничному!';uk='Неможливо автоматично виключати зі складу рядків 2010 та 2030 суму заборгованості по лікарняним!'", КодЯзыка);
...
мСоставПоказателей = "СоставПоказателей2012Мес";
