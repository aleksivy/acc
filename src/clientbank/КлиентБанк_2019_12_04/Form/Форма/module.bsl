...
Процедура ПриИзмененииФормыОбработки()
	...
	Если ПоддерживаемыеФорматы.Импорт Тогда
		...
		КодировкаФайлаИмпорт= СписокТиповФайлов.НайтиПоЗначению(1);
	...
	Если ПоддерживаемыеФорматы.Экспорт Тогда
		...
		//КодировкаФайлаИмпорт = СписокТиповФайлов.НайтиПоЗначению(1);
		КодировкаФайлаЭкспорт = СписокКодировокФайлов[0].Значение;
...
Процедура ИмпортВыписки()
	...
	Для Каждого Строка Из тзВыписка Цикл
		...
		Если НЕ ЗначениеЗаполнено(НоваяСтрока.СчетКонтрагента) Тогда
			...
			//Если ПравоДоступа("Добавление", Метаданные.Справочники.БанковскиеСчета, ПользователиИнформационнойБазы.ТекущийПользователь()) Тогда
			Если ПравоДоступа("Добавление", Метаданные.Справочники.БанковскиеСчета) Тогда
				НоваяСтрока.СчетКонтрагента = ДобавитьСчетКонтрагента(Контрагент, Строка.РСчет, Строка.МФО);
			Иначе	
				Сообщить("Необходимо создать банковский счет - " + СокрЛП(Строка.РСчет) + " в банке с МФО - " + Строка.МФО + " для контрагента - " + Контрагент + " (Код ЕДРПОУ " + Контрагент.КодПоЕДРПОУ + ")"); 
			КонецЕсли;
...
Процедура ПрочитатьСуществующиеПлатежи()
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяСтрока.ТМЦ				= Строка(Выборка.Договор.Номенклатура);
		...
		НоваяСтрока.Количество 		= Выборка.Ссылка.РасшифровкаПлатежа[0].Количество;
		НоваяСтрока.Ответственный 	= Выборка.Ответственный;
		НоваяСтрока.НалоговоеНазначение	= Выборка.Ссылка.РасшифровкаПлатежа[0].НалоговоеНазначение;
		...
		НоваяСтрока.Флаг = ?(ТипЗнч(Выборка.Ссылка)=Тип("ДокументСсылка.ПлатежноеПоручениеИсходящее"), Ложь, Истина);
...
Процедура ЗаполнитьРасшифровкуПлатежа(РасшифровкаПлатежа, Сделка, Договор, Сумма, Количество ,Строка, докОб, докОбМетаданные, НалоговоеНазначение)
	...
	РасшифровкаПлатежа.Количество				= Строка.Количество;
	РасшифровкаПлатежа.НалоговоеНазначение		= Строка.НалоговоеНазначение;
	...
	Если Конфигурация = "УПП" Или Конфигурация = "БУ" Или Конфигурация = "УТП" Тогда	
		...
		Если ПолучитьИмяОбъектаКонфигурации("ТипДокументаВыпискаПриход").НайтиПоЗначению(Строка.ВидДокумента) <> Неопределено Тогда 	
			...
			Если Конфигурация = "УТП" Или Конфигурация = "БУ" Тогда
				...
				РасшифровкаПлатежа.НалоговоеНазначение 			= Справочники.НалоговыеНазначенияАктивовИЗатрат.НКУ_ХозДеятельность;
...
Процедура СформироватьВыписки()
	...
	Для Каждого Строка Из тчИмпорт Цикл
		...
		Если Не Строка.Флаг и (Строка.Статус = СтатусыСтрокИмпорта.СуществующийПлатежНеобходимоОплатить) Тогда
...
//---------------------------------------------------------------------------------------------------			
		...
		докОб = Док.СоздатьДокумент();
			...
			докОб.ДоговорКонтрагента 	= Строка.Договор;
			...
			Если  ДокОб.Метаданные().Имя= "ПлатежноеПоручениеВходящее" Тогда
			//в таб части РасшифровкаПлатежа
			...
			//Если Строка.НайденныеСделкиДляЗаполнения = "" ИЛИ (ТипЗнч(НайденныеСделкиДляЗаполнения) = Тип("ТаблицаЗначений") И НайденныеСделкиДляЗаполнения.Количество() = 0) Тогда
			 Если  ДокОб.Метаданные().Имя= "ПлатежноеПоручениеВходящее" Тогда
				...
				ЗаполнитьРасшифровкуПлатежа(РасшифровкаПлатежа, Строка.Сделка, Строка.Договор, докОб.СуммаДокумента,Строка.Количество ,Строка, докОб, докОбМетаданные, Строка.НалоговоеНазначение);
			КонецЕсли;	
			...
			//		КонецЕсли;	
			//		РасшифровкаПлатежа = докОб.РасшифровкаПлатежа.Добавить();
			//		ЗаполнитьРасшифровкуПлатежа(РасшифровкаПлатежа, СтрокаИзТаблицыСделок.Сделка, СтрокаИзТаблицыСделок.Сделка.ДоговорКонтрагента, СтрокаИзТаблицыСделок.СуммаКБ,СтрокаИзТаблицыСделок.Количество ,Строка, докОб, докОбМетаданные, СтрокаИзТаблицыСделок.НалоговоеНазначение);
			//	КонецЦикла;	
			//КонецЕсли;	
...
//
	тзПлатежныеПоручения.Колонки.Добавить("ВидОперации");
//
	...
	Для Каждого Строка Из тчЭкспорт Цикл
		Если НЕ Строка.Флаг Тогда
			Продолжить;
		КонецЕсли;
		ТекДок = Строка.Документ.ПолучитьОбъект();
		ТекДокМетаданные = ТекДок.Метаданные();
		ТекДок.ОтправленоВБанк = Истина;
		...
		НовСтрока.ВидОперации 	= Строка.Документ.ВидОперации;
		...
		Иначе
			НовСтрока.СуммаНДС = Строка.Сумма-(Строка.Сумма * 100 / (100 + СтавкаНДС));
		КонецЕсли;
		
	КонецЦикла;
	// Надо вставить "галочку" для проведения выгружаемых платежей
	Если Ложь Тогда
		Для Каждого Строка Из тчЭкспорт Цикл
...
Процедура ПрочитатьСуществующиеПП()
	...
	|   ОтправленоВБанк КАК ОтправленоВБанк,
	|	ДоговорКонтрагента КАК Договор";
	...
	Пока Выборка.Следующий() Цикл
		...
		КонецЕсли;	
		МфоКлиента=Выборка.Ссылка.СчетКонтрагента.Банк.Код;
		Дата=ДатаПППо;
		
		НоваяСтрока.Документ		= Выборка.Ссылка;
		
		ЕСли ПроверкаМФО(МфоКлиента,Дата) ТОгда
			НоваяСтрока.Флаг = Ложь;
				Иначе	
			НоваяСтрока.Флаг = Истина;
		КонецЕсли;
...
Процедура тпЭкспортПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	Если Не ДанныеСтроки.Флаг Тогда
