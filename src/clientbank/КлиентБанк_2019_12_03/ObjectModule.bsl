...
Процедура ВосстановитьЗначениеНастроек() Экспорт
	...
	//НастройкиОбработки.Вставить("РасчетныйСчетПоУмолчанию", Справочники.БанковскиеСчета.НайтиПоКоду(ЭлементыФормы.ДокументСписок.ТекущиеДанные.СчетОрганизации.Банк.Код)));
...
Функция ДобавитьСчетКонтрагента(Контрагент, НомерСчета, МФО) Экспорт
	...
	//Если Не ПравоДоступа("Добавление", Метаданные.Справочники.БанковскиеСчета, ПользователиИнформационнойБазы.ТекущийПользователь()) Тогда
	Если Не ПравоДоступа("Добавление", Метаданные.Справочники.БанковскиеСчета) Тогда
		Сообщить("Необходимо создать банковский счет - " + СокрЛП(НомерСчета) + " в банке с МФО - " + МФО + " для контрагента - " + Контрагент + " (Код ЕДРПОУ " + Контрагент.КодПоЕДРПОУ + ")"); 
		Возврат Справочники.БанковскиеСчета.ПустаяСсылка();
	КонецЕсли;
	спрБанкСчет = Справочники.БанковскиеСчета.СоздатьЭлемент();
	спрБанкСчет.Владелец 		= Контрагент;
	спрБанкСчет.Наименование 	= СокрЛП(НомерСчета);
...
Функция НайтиСчетКонтрагента(НомерСчета, Контрагент) Экспорт
	...
	Если Лев(НомерСчета,2) = "UA" Тогда
		СчетВФорматеIBAN = Истина;
	Иначе
		СчетВФорматеIBAN = Ложь;
	КонецЕсли;
	
	Если СчетВФорматеIBAN Тогда
		лНомерСчетаНадо = Прав(НомерСчета, 19);
		Пока Лев(лНомерСчетаНадо,1) = "0" Цикл
			лНомерСчетаНадо = Прав(лНомерСчетаНадо, СтрДлина(лНомерСчетаНадо)-1);
		КонецЦикла;	
	Иначе
		лНомерСчетаНадо = НомерСчета;
	КонецЕсли;	
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |			ИЛИ БанковскиеСчета.НомерСчета = &НомерСчетаСтарый
	               |			ИЛИ БанковскиеСчета.НомерСчетаСтарый = &НомерСчетаСтарый)";
	
	Запрос.Текст = ТекстЗапроса;
	Запрос.УстановитьПараметр("НомерСчета",	НомерСчета);
	Запрос.УстановитьПараметр("НомерСчетаСтарый",	лНомерСчетаНадо);
...
Функция ЗаменитьСимволы(Стр) Экспорт
	...
	СтрВозврата = СтрЗаменить(СтрВозврата, "°", "Ї");
	СтрВозврата = СтрЗаменить(СтрВозврата, "*", "");
	Возврат СтрВозврата;
КонецФункции
...
Функция ПроверкаМФО(МфоКлиента,Дата) Экспорт
	ЗапросМФО = Новый Запрос;
	ЗапросМФО.УстановитьПараметр("МфоКлиента", СокрЛП(МфоКлиента));
	ЗапросМФО.УстановитьПараметр("Дата", Дата);
	ЗапросМФО.Текст=
	"ВЫБРАТЬ
	|	ПроверкаМФОБанка.Заблокировано
	|ИЗ
	|	РегистрСведений.ПроверкаМФОБанка.СрезПоследних(&Дата, МФОБанка = &МфоКлиента) КАК ПроверкаМФОБанка
	|";
	Выборка  =  ЗапросМФО.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Возврат Выборка.Заблокировано;
	Иначе
		Возврат Ложь;
	КонецЕсли;
КонецФункции;
