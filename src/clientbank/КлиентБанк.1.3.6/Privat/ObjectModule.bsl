...
Функция ИмпортDBF(Параметры)
	...
	Пока НЕ ФайлDBF.ВКонце() Цикл
		...
		Если ФайлDBF.DATE <> Параметры.ДатаДок И СокрЛП(ФайлDBF.DATE) <> Формат(Параметры.ДатаДок,"ДФ=yyyyMMdd") И Формат(ФайлDBF.DATE,"ДФ=dd.MM.yyyy") <> Формат(Параметры.ДатаДок,"ДФ=dd.MM.yyyy") Тогда
			ФайлDBF.Следующая();
			Продолжить;
		КонецЕсли;	
		
		// проверка на совпадение расчетного счета
		Если СокрЛП(Формат(ФайлDBF.COUNT_A, "ЧГ=0")) = Параметры.РСчет Тогда
			НовСтрока = тзВыписка.Добавить();
			НовСтрока.РСчет			= СокрЛП(Формат(ФайлDBF.COUNT_B,"ЧГ=0"));
			...
			НовСтрока.Приход		= 0;
            НовСтрока.Расход		= Макс(ФайлDBF.SUMMA, -ФайлDBF.SUMMA);
		ИначеЕсли СокрЛП(Формат(ФайлDBF.COUNT_B, "ЧГ=0")) = Параметры.РСчет Тогда
			НовСтрока = тзВыписка.Добавить();
			НовСтрока.РСчет			= СокрЛП(Формат(ФайлDBF.COUNT_A,"ЧГ=0"));
			НовСтрока.МФО 			= СокрЛП(Формат(ФайлDBF.MFO_A,"ЧГ=0"));
 			НовСтрока.ОКПО			= СокрЛП(Формат(ФайлDBF.OKPO_A,"ЧГ=0"));
			НовСтрока.Контрагент	= СокрЛП(ФайлDBF.NAME_A);
			НовСтрока.Приход		= Макс(ФайлDBF.SUMMA, -ФайлDBF.SUMMA);
            НовСтрока.Расход		= 0;
		Иначе	
			ФайлDBF.Следующая();
			Продолжить;
