...
Функция ИмпортDBF(Параметры)
	...
	//	ДатаН = Лев(ФайлDBF.PAY_DATE,4)+ Сред(ФайлDBF.PAY_DATE,6,2)+ Прав(ФайлDBF.PAY_DATE,2);
		//ДатаН = ФайлDBF.DAT_K;
		// проверка на дату платежа
		Если  ФайлDBF.DAT<> Параметры.ДатаДок И СокрЛП(ФайлDBF.DAT) <> Формат(Параметры.ДатаДок,"ДФ=yyyyMMdd") Тогда
			ФайлDBF.Следующая();
			Продолжить;
		КонецЕсли;	
		
		// проверка на совпадение расчетного счета
		Если Число(ФайлDBF.OPERAT) = 1 Тогда        //расход
		...
		ИначеЕсли Число(ФайлDBF.OPERAT) = 2 Тогда   //приход
			НовСтрока = тзВыписка.Добавить();
		...
		НовСтрока.НомерПП 			= ФайлDBF.N_DOC;
