...
Функция ЭкспортTXT(Параметры)
	...
	Для Каждого Строка из Параметры.тзПлатежныеПоручения Цикл
		...
		ТекСтрока = ТекСтрока + Лев(Строка.Контрагент + СтрокаПробелов, 38); //Позиция 129-166
		ТекСтрока = ТекСтрока + Лев(Строка.Содержание + СтрокаПробелов, 160);//Позиция 167-326
		...
		ТекСтрока = ТекСтрока + Лев("" + СтрокаПробелов, 3); //Дополнительные реквизиты   387-389
		ТекСтрока = ТекСтрока + Лев("" + СтрокаПробелов, 2); //Дополнительные реквизиты   390-391
		ТекСтрока = ТекСтрока + Лев("" + СтрокаПробелов, 14); //Дополнительные реквизиты  392-405
		...
		ТекСтрока = ТекСтрока + Лев(Строка.ОКПО + СтрокаПробелов, 14); //Позиция 406-419
