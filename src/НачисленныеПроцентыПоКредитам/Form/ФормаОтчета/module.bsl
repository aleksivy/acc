...
Процедура УстановитьПараметрыОтборы()
	...
	УстановитьПараметр(КомпоновщикНастроек, "Дата", КонецДня(ДатаКон) );
КонецПроцедуры

Процедура ПриОткрытии()
	Если Не ЗначениеЗаполнено(ДатаКон) Тогда ДатаКон = НачалоКвартала(ТекущаяДата())-1; КонецЕсли;
	Если Не ЗначениеЗаполнено(ДатаНач) или (ДатаКон >= ДатаКон) Тогда ДатаНач = НачалоКвартала(ДатаКон); КонецЕсли;
	
	УстановитьПараметрыОтборы();
КонецПроцедуры

Процедура ДатаКонПриИзменении(Элемент)
	ДатаНач = НачалоКвартала(ДатаКон);
	УстановитьПараметрыОтборы();
КонецПроцедуры
